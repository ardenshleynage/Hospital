...
if ($searcha_lower == "autoriser") {
    $sql = "SELECT * FROM `users` WHERE `statut` = 1"; // Utiliser IS NULL pour vérifier si le champ est null
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $searchallusers = $stmt->fetchAll(PDO::FETCH_ASSOC);

} elseif ($searcha_lower == "bloquer") {
    $sql = "SELECT * FROM `users` WHERE `statut` = 0"; // Utiliser IS NULL pour vérifier si le champ est null
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $searchallusers = $stmt->fetchAll(PDO::FETCH_ASSOC);
} else {
